...
def get_value_pollutant(location: str, date: str, time: int, pollutant: str) -> float:
    """
    Recupera il valore di concentrazione di un inquinante per una specifica stazione, data e ora.

    Args:
        location (str): Nome esatto della stazione di monitoraggio.
        date (str): Data nel formato YYYY-MM-DD.
        time (int): Ora del giorno (0–23).
        pollutant (str): Nome dell'inquinante (es. NO2, PM10, PM2,5, PM2.5, O3).

    Returns:
        float: Valore della concentrazione dell'inquinante in µg/m³.
    """
    # Normalizza il nome dell'inquinante
    pollutant_norm = pollutant.replace(",", ".")
    df = load_file(date)
    # Normalizza anche i nomi degli inquinanti nel DataFrame
    df["pollutant"] = df["pollutant"].replace({"PM2,5": "PM2.5"})
    return df[
        (df["date"].dt.strftime("%Y-%m-%d") == date) &
        (df["date"].dt.hour == time) &
        (df["station_name"] == location) &
        (df["pollutant"] == pollutant_norm)
    ].iloc[0]["value"]
...
def get_pollutant_name():
    """
    Restituisce la lista degli inquinanti monitorati.

    Returns:
        list: Lista dei nomi degli inquinanti.
    """
...
def get_pollutant_description(pollutant: str) -> str:
    ...
    return descriptions.get(pollutant_norm, "Descrizione non disponibile per questo inquinante.")
...
def send_request(request_user):
    ...
    try:
        ...
        print(f"Errore nella richiesta a Gemini: {e}")
        return "Si è verificato un errore nel chatbot. Riprova più tardi."
